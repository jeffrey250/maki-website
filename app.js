...
function addArticle(){const p=document.querySelector('.editor-panel');const title=p.querySelector('[data-article=title]')?.value.trim();if(!title)return;const image=p.querySelector('[data-article=image]')?.value.trim()||A.blogHero;const excerpt=p.querySelector('[data-article=excerpt]')?.value.trim()||'New MĀKI article.';const body=p.querySelector('[data-article=body]')?.value.trim()||excerpt;const slug=slugify(title);const content=body.split(/\n\s*\n/).map(text=>({tag:'p',text:text.trim()})).filter(x=>x.text);const article={custom:true,slug,title,image,category:'Environmental Services',date:new Date().toLocaleDateString('en-NZ',{month:'short',day:'numeric',year:'numeric'}),readTime:'1 min read',excerpt,content};localStorage.setItem('makiArticles',JSON.stringify([article,...getArticles().filter(a=>a.slug!==slug)]));render()}
